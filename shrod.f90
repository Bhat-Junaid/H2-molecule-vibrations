! This program calculates variation of de as a function of e
PROGRAM SCHROD
    IMPLICIT NONE
    INTEGER (KIND=4), PARAMETER :: N=3001
    INTEGER (KIND=4):: NCMAX, IFLAG, M, JLEV, NCOUNT, NODE,I
    REAL (KIND=8), DIMENSION(0:N):: V,F,RR
    REAL (KIND=8), DIMENSION(0:200):: E_values, DE_values
    REAL (KIND=8):: R0,RN,A,B,EPS,TOL,FINIT,ENERG,H,HC,E,DE
    
    DATA R0/0.1D0/, RN/5.6D0/, A/1.39241/, B/1.4e-3/, EPS/1.0D-04/, TOL/1.0D-4/,&
    FINIT/0.D0/, ENERG/-0.98D0/, NCMAX/30/, IFLAG/1/, M/11/, JLEV/0/
    ...
    ! open the file to put all the values of e and deltae (question 4)
    OPEN(UNIT=1,FILE="E_DE_v0.txt")
    DO I=0,200
        E= ENERG + I*0.004 ! to increase the energy at each step 
        NCOUNT=0
        ...
        E_values(I) = E
        DE_values(I) = DE
        WRITE(1,100) E_values(I),DE_values(I)
        100 FORMAT(2E15.7)
    ENDDO
    CLOSE(1)
    END
